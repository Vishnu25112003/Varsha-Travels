...
export default function Booking() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Persist booking locally so admin Bookings page can read it
    try {
    ...
    // Send booking details by email via backend (same mail config as contact form)
    try {
      setIsSubmitting(true);

      const subject = `New booking request - ${
        bookingWithMeta.destination || "Destination not specified"
      }`;

      const messageLines = [
        `New booking request from Varsha Travels website.`,
        "",
        `Name: ${bookingWithMeta.fullName}`,
        `Email: ${bookingWithMeta.email}`,
        `Phone: ${bookingWithMeta.phone}`,
        `Destination: ${bookingWithMeta.destination}`,
        `Vehicle: ${bookingWithMeta.vehicle}`,
        `Pickup Date: ${bookingWithMeta.pickupDate}`,
        `Dropoff Date: ${bookingWithMeta.dropoffDate}`,
        `Passengers: ${bookingWithMeta.passengers}`,
        "",
        "Special requests:",
        bookingWithMeta.specialRequests || "(none)",
      ];
      ...
      if (!res.ok) {
        throw new Error(body.message || "Failed to send booking email");
      }

      console.log("Booking submitted:", bookingWithMeta);
      alert(
      ...
      alert(
        error.message ||
          "Failed to send your booking details. Please try again later.",
      );
    } finally {
